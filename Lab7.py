...
# Date of last modification: 04/29/2019
...
def draw_maze2(walls,maze_rows,maze_cols,cell_nums=False):
    fig, ax = plt.subplots()

    for i in range(len(walls)):
        for a in walls[i]:
            ax.plot([i], [a], linewidth=2, color='k')
    ...
    for r in range(maze_rows):
        for c in range(maze_cols):
            ...
            if r!=maze_rows-1:
                ...
                #print([cell,cell+1],' ',[cell,cell+maze_cols])
       # print()
    return w
...
def union_by_size(S, i, j):
    ...
    if ri != rj:  # if i and j do not belongto the same set
...
'''def maceAdjacency(macesize):
    adj = []
    for i in range(macesize):
        adj.append([i])
    return adj'''

# Breadth first search using queues
def breadth_first_search (G, v, u):
    visited = [False] * len(G)  # boolean list to store node visited
    #print('len g os : ',len(G))
    queue = []  # list store value of node visited
    queue.append(v)  # appending value of node visited
    visited[v]= True  # marking node visited as true

    while queue:
        v =  queue.pop(0)  # removing value of node visited from list
        print(v, end =' ')
        for i in G[v]:
            if visited[i]== False:  # checking if node has been visited
                queue.append(i)  # appending visited node to list
                visited[i]=True  # marking node visited as true
               # print('i is ',i)
                #print('u is ', u)
                if i == u:
                    print('goal found')
...
# Deptth first search using recursion
def depth_first_search_recursion(G, vertex, visited):
    if vertex not in visited: # checking if vertex has been visited
        visited.append(vertex)  # appending value of node visited
        for n in G[vertex]:
            depth_first_search_recursion(G, n, visited)  # recursive call to check and append vertices visited
    return visited

# Depth first search using stack
def depth_first_search_stack(G, v, u):
    visited = [False] * len(G)  # boolean list to store node visited
    #print('len g os : ',len(G))
    stack = []  # list store value of node visited
    stack.append(v)  # appending value of node visited
    visited[v]= True  # marking node visited as true

    while stack:
        v =  stack.pop(-1)  # removing value of node visited from list
        print(v, end =' ')
        for i in G[v]:
            if visited[i]== False:  # checking if node has been visited
                stack.append(i)  # appending visited node to list
                visited[i]=True  # marking node visited as true
                if i == u:
                    print('goal found')
                    #break
...
def BuildMazeUnionStd(Wall, S, TotalCells, cellsRemoved):
    if cellsRemoved < (TotalCells - 1):
        while NumSets(S) > (TotalCells-cellsRemoved):  # Checking if number of sets in maze more than 1
            rand = random.randint(0, len(Wall) - 1)  # Creating a random number from 0 to last cell number
            if InSameSet(S, Wall[rand][0],
                         Wall[rand][1]) == False:  # checking if cells with random number are in same set
    ...
    elif cellsRemoved == (TotalCells - 1):
        ...
        counter = 0
        while  counter < (cellsRemoved) :  # Checking if number of sets in maze more than 1
            rand = random.randint(0, len(Wall) - 1)  # Creating a random number from 0 to last cell number
            if InSameSet(S, Wall[rand][0],
                         Wall[rand][1]) == False:  # checking if cells with random number are in same set
                union(S, Wall[rand][0], Wall[rand][1])  # uniting the cells if they are in different sets
...
def BuildMazeUnionStd2(Wall, S, TotalCells, cellsRemoved):
    Wall2 =[]
    ...
    while NumSets(S)>1:  # Checking if number of sets in maze more than 1
        ...
        if InSameSet(S, Wall[rand][0], Wall[rand][1])== False:  # checking if cells with random number are in same set
            ...
            #Wall.pop(rand)  # Removing the list from the maze
            Wall2[Wall[rand][0]].append(Wall[rand][1])
    return Wall2

def BuildAdjacencyMaze(Wall, S, TotalCells, cellsRemoved):
        newWall = []
        for i in range(len(S)):
            newWall.append([])
        while NumSets(S) > 1:  # Checking if number of sets in maze more than 1
            ...
            if InSameSet(S, Wall[rand][0],
                        Wall[rand][1]) == False:  # checking if cells with random number are in same set
                        union_by_size(S, Wall[rand][0], Wall[rand][1])
                        newWall[Wall[rand][0]].append(Wall[rand][1])
                        Wall.pop(rand)
        draw_maze(Wall, maze_rows, maze_cols)
        return newWall
plt.close("all")

maze_rows = 10  # Number of rows in maze
maze_cols = 10  # Number of columns in maze
TotalCells = maze_cols*maze_rows
print('The total number of cells in the maze is: ',TotalCells)
cellsRemoved = int(input('Enter the number of cells you will like to remove: '))
...
#print('lengt if wall is: ',len(walls))
...
newMaze = BuildMazeUnionStd(walls, S,TotalCells, cellsRemoved)  # Maze with simple path using standard union
print('length of new maze: ',len(newMaze))
...
walls3 = wall_list(maze_rows,maze_cols)
S3 = np.zeros(maze_rows*maze_cols, dtype=np.int) - 1
#adjMace = maceAdjacency(TotalCells)
newMaze3 = BuildAdjacencyMaze(walls3, S3, TotalCells, cellsRemoved)
print('The adjency list of the mace is: ')
print(newMaze3)
#print(newMaze3[0][0])
#print([len(newMaze)-1][-1])
print()
print('The Breadth First Search solution is: ')
breadth_first_search(newMaze3,newMaze3[0][0],newMaze3[len(newMaze)-1][-1])
#breadth_first_search(walls3,walls3[0][0],walls3[80][1])
#draw_maze(newMaze3,maze_rows,maze_cols,cell_nums=True)'''
print()
print('The Depth First Search with Stack solution is: ')
depth_first_search_stack(newMaze3,newMaze3[0][0],newMaze3[len(newMaze)-1][-1])
print()
print('The Depth First Search with Recursion solution is: ')
dfsrecur =depth_first_search_recursion(newMaze3,newMaze3[0][0],[])
print(dfsrecur)
